    ...
    for (i = idx - 1; i < nx + 1; i += offsetX)
        for (j = idy - 1; j < ny + 1; j += offsetY)
            for (k = idz - 1; k < nz + 1; k += offsetZ) {
                data[_ind(i, j, k)] = ((i != -1 && j != -1 && k != -1 && i != nx && j != ny && k != nz))
                                      * fabs(next[_ind(i, j, k)] - data[_ind(i, j, k)]);
            }
}
...
int main(int argc, char *argv[]) {
    ...
    int _size_maximum = (max(npx, max(npy, npz)) * max(npx, max(npy, npz)) + 2);
    double* gpu_data, *gpu_next, *gpu_edge_xy;
    CUDA_ERROR(cudaMalloc((void**)&gpu_data, sizeof(double) * (npx + 2) * (npy + 2) * (npz + 2)));
    CUDA_ERROR(cudaMalloc((void**)&gpu_next, sizeof(double) * (npx + 2) * (npy + 2) * (npz + 2)));
    CUDA_ERROR(cudaMalloc((void**)&gpu_edge_xy, sizeof(double) * _size_maximum));
    // CUDA_ERROR(cudaMalloc((void**)&gpu_edge_xz, sizeof(double) * npx * npz));
    // CUDA_ERROR(cudaMalloc((void**)&gpu_edge_yz, sizeof(double) * npy * npz));
    ...
    edge_xy = (double *)malloc(sizeof(double) * _size_maximum);
    // edge_xz = (double *)malloc(sizeof(double) * (npx * npz + 2));
    // edge_yz = (double *)malloc(sizeof(double) * (npy * npz + 2));

    // fill_n(edge_xy, _size_maximum, 0.0);
    // fill_n(edge_xz, npx * npy, 0.0);
    // fill_n(edge_yz, npx * npy, 0.0);

    // CUDA_ERROR(cudaMemcpy(gpu_edge_xy, edge_xy, sizeof(double) * _size_maximum, cudaMemcpyHostToDevice));
    ...
    do {
        if (ib + 1 < blocks[on_x]) {
            kernel_copy_edge_yz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, npx - 1, true, u0);
            CUDA_ERROR(cudaGetLastError());
            CUDA_ERROR(cudaMemcpy(edge_xy, gpu_edge_xy, sizeof(double) * _size_maximum, cudaMemcpyDeviceToHost));
            MPI_Send(edge_xy, npy * npz, MPI_DOUBLE, _ib(ib + 1, jb, kb), 0, MPI_COMM_WORLD);
        }

        // Data recieve
        if (ib > 0) {
            MPI_Recv(edge_xy, npy * npz, MPI_DOUBLE, _ib(ib - 1, jb, kb), 0, MPI_COMM_WORLD, &status);
            CUDA_ERROR(cudaMemcpy(gpu_edge_xy, edge_xy, sizeof(double) * _size_maximum, cudaMemcpyHostToDevice));
            kernel_copy_edge_yz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, - 1, false, u0);
        } else {
        ...
        // Back
        if (jb + 1 < blocks[on_y]) {
            kernel_copy_edge_xz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, npy - 1, true, u0);
            CUDA_ERROR(cudaGetLastError());
            CUDA_ERROR(cudaMemcpy(edge_xy, gpu_edge_xy, sizeof(double) * _size_maximum, cudaMemcpyDeviceToHost));
            MPI_Send(edge_xy, npx * npz, MPI_DOUBLE, _ib(ib, jb + 1, kb), 0, MPI_COMM_WORLD);
        }

        if (jb > 0) {
            MPI_Recv(edge_xy, npx * npz, MPI_DOUBLE, _ib(ib, jb - 1, kb), 0, MPI_COMM_WORLD, &status);
            CUDA_ERROR(cudaMemcpy(gpu_edge_xy, edge_xy, sizeof(double) * _size_maximum, cudaMemcpyHostToDevice));
            kernel_copy_edge_xz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, - 1, false, u0);
        } else {
        ...
        // Up
        if (kb + 1 < blocks[on_z]) {
            kernel_copy_edge_xy<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, npz - 1, true, 0.0);
            CUDA_ERROR(cudaGetLastError());
            CUDA_ERROR(cudaMemcpy(edge_xy, gpu_edge_xy, sizeof(double) * _size_maximum, cudaMemcpyDeviceToHost));
        ...
        // Left
        if (ib > 0) {
            kernel_copy_edge_yz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, 0, true, u0);
            CUDA_ERROR(cudaGetLastError());
            CUDA_ERROR(cudaMemcpy(edge_xy, gpu_edge_xy, sizeof(double) * _size_maximum, cudaMemcpyDeviceToHost));
            MPI_Send(edge_xy, npy * npz, MPI_DOUBLE, _ib(ib - 1, jb, kb), 0, MPI_COMM_WORLD);
        }
        ...
        if (ib + 1 < blocks[on_x]) {
            MPI_Recv(edge_xy, npy * npz, MPI_DOUBLE, _ib(ib + 1, jb, kb), 0, MPI_COMM_WORLD, &status);
            CUDA_ERROR(cudaMemcpy(gpu_edge_xy, edge_xy, sizeof(double) * _size_maximum, cudaMemcpyHostToDevice));
            kernel_copy_edge_yz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, npx, false, u0);
        } else {
        ...
        // Front
        if (jb > 0) {
            kernel_copy_edge_xz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, 0, true, u0);
            CUDA_ERROR(cudaGetLastError());
            CUDA_ERROR(cudaMemcpy(edge_xy, gpu_edge_xy, sizeof(double) * _size_maximum, cudaMemcpyDeviceToHost));
            MPI_Send(edge_xy, npx * npz, MPI_DOUBLE, _ib(ib, jb - 1, kb), 0, MPI_COMM_WORLD);
        }
        ...
        if (jb + 1 < blocks[on_y]) {
            MPI_Recv(edge_xy, npx * npz, MPI_DOUBLE, _ib(ib, jb + 1, kb), 0, MPI_COMM_WORLD, &status);
            CUDA_ERROR(cudaMemcpy(gpu_edge_xy, edge_xy, sizeof(double) * _size_maximum, cudaMemcpyHostToDevice));
            kernel_copy_edge_xz<<<gblocks, threads>>>(gpu_edge_xy, gpu_data, npx, npy, npz, npy, false, u0);
        } else {
        ...
        kernel_error<<<dim3(8, 8, 8), dim3(32, 4, 4)>>> (gpu_next, gpu_data, npx, npy, npz);
        CUDA_ERROR(cudaGetLastError());
        ...
        // Cast to thrust
        thrust::device_ptr< double > pointers = thrust::device_pointer_cast(gpu_data);
